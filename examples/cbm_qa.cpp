...
//   if(GetCbmTrackCuts(rec_tracks) == nullptr)
//     std::cout << "NULLPTR!!!\n";
//   if(GetCbmTrackCuts(rec_tracks) != nullptr)
//     std::cout << "NOT NULLPTR!!!\n";
  
//   task->AddBranchCut(GetCbmTrackCuts(rec_tracks));
//   task->AddBranchCut(GetCbmMcTracksCuts(sim_particles));
//   task->AddBranchCut(GetCbmTofHitsCuts(tof_hits));
  task->SetEventCuts(GetCbmEventCuts(rec_event_header));
...
void RecEventHeaderQA(QA::Task& task)
{
//   SimpleCut vtx_x_cut = RangeCut((rec_event_header + ".vtx_x").c_str(), -0.5, 0.5);
//   SimpleCut vtx_y_cut = RangeCut((rec_event_header + ".vtx_y").c_str(), -0.5, 0.5);
//   SimpleCut vtx_z_cut = RangeCut((rec_event_header + ".vtx_z").c_str(), -0.03, 0.03);
//   SimpleCut vtx_chi2_cut = RangeCut((rec_event_header + ".vtx_chi2").c_str(), 0.8, 1.7);
// 
//   Cuts* event_cuts = new Cuts("CbmGoodEvent", {vtx_x_cut, vtx_y_cut, vtx_z_cut, vtx_chi2_cut});
...
//   task.AddH1({"x_{vertex} (cm)", {rec_event_header, "vtx_x"}, {QA::gNbins, -1, 1}}, event_cuts);
//   task.AddH1({"y_{vertex} (cm)", {rec_event_header, "vtx_y"}, {QA::gNbins, -1, 1}}, event_cuts);
//   task.AddH1({"z_{vertex} (cm)", {rec_event_header, "vtx_z"}, {QA::gNbins, -1, 1}}, event_cuts);
//   task.AddH1({"#chi^{2}_{vertex fit}", {rec_event_header, "vtx_chi2"}, {QA::gNbins, 0, 5}}, event_cuts);
// 
//   task.AddH1({"E_{PSD} (GeV)", {rec_event_header, "Epsd"}, {QA::gNbins, 0, 60}}, event_cuts);
//   task.AddH1({"M_{tracks}", {rec_event_header, "M"}, {800, 0, 800}}, event_cuts);
//   task.AddH1({"Event ID", {rec_event_header, "evt_id"}, {QA::gNbins, 0, 2000}}, event_cuts);
// 
//   task.AddH2({"x_{vertex} (cm)", {rec_event_header, "vtx_x"}, {QA::gNbins, -1, 1}},
//              {"y_{vertex} (cm)", {rec_event_header, "vtx_y"}, {QA::gNbins, -1, 1}}, event_cuts);
// 
//   task.AddH2({"x_{vertex} (cm)", {rec_event_header, "vtx_x"}, {QA::gNbins, -1, 1}},
//              {"x_{vertex} (cm)", {sim_event_header, "vtx_x"}, {QA::gNbins, -1, 1}}, event_cuts);
//   task.AddH2({"y_{vertex} (cm)", {rec_event_header, "vtx_y"}, {QA::gNbins, -1, 1}},
//              {"y_{vertex} (cm)", {sim_event_header, "vtx_y"}, {QA::gNbins, -1, 1}}, event_cuts);
//   task.AddH2({"z_{vertex} (cm)", {rec_event_header, "vtx_z"}, {QA::gNbins, -1, 1}},
//              {"z_{vertex} (cm)", {sim_event_header, "vtx_z"}, {QA::gNbins, -1, 1}}, event_cuts);
}
...
int main(int argc, char* argv[]){
    ...
    std::cout << " ./cbm_qa filename" << std::endl;
    exit(EXIT_FAILURE);
  }
